...
class PositionalEncoder(nn.Module):
    '''add the voca location'''
    def __init__(self, position, d_model):
        super().__init__()

        self.d_model = d_model  # original voca dimension
...
def scaled_dot_product_attention(query, key, value, mask):
  ...
  # Masking
  # when softmax pass, they will be near 0, 
  if mask is not None:
    logits += (mask * -1e9)

  # soft-max function will be implemented
  # attention weight : (batch_size, num_heads, query's sentence length, key's sentence length)
...
Q.build_vocab(trainset.Q, trainset.A, min_freq = 50) # 단어 집합 생성
...
# TransformerBlock module initialize
